pipeline {
  ...
  stages {
    ...
    stage('Fetching Database and storing artifacts') {
      steps {
          script{
            def dbList=sh(script:"sudo kubectl --kubeconfig=${KUBECONFIG} exec postgresdb-0 -- PGPASSWORD=Sahil@123 psql -h localhost -U sahil -c 'SELECT datname FROM pg_database' -t", returnStdout:true).trim().split('\n')
